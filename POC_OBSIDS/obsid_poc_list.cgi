...
#################################################################################################
#			                                                									#
#	obsid_poc_list.cgi: display who is in charge for a give obsid/seq                			#
#												                                                #
...
print  '<body style="background-color:#FFFFE0; font-family:serif, sans-serif;font-size:12pt; color:black;" >';
...
#####$input_no = param('input_obsid');

#if($check =~ /Submit/ || $input_no ne ''){
#    if($input_no eq ''){
...
print '<h2 style="background-color:blue; color:#FFFFE0">USINT Contact For A Given OBSID/Sequence Number</h2>';

print '<div style="text-align:right"><a href="https://cxc.cfa.harvard.edu/mta/CUS/Usint/poc_obsid_list_ver2.cgi">';
...
if($input_no =~ /\d/){
	...
	if($chk == 0){
		...
		print "ObsID: <b>$obsid</b> / ";
		print "Sequene Number: <b>$seqno</b>";
		if($sts ne ''){
		...
		if($usint eq '' && ($status =~ /scheduled/ || $status =~ /unobserved/)){
			...
			print "<td style='text-align:center'><a href='https://cxc.cfa.harvard.edu/mta/CUS/Usint/poc_obsid_list_ver2.cgi\?$id[$pos]'>$name[$pos]</a>", '</td>';
		...
		if($cnt1 > 0 || $cnt2 > 0){
			...
			if($cnt1 > 0){
				...
				foreach $ent (@group_obsid){
					...
					print "<tr><td><b>$ent</b></td>";
					print "<td><b>$seqno</b></td>";
	...
	if($disp > 1){
		...
		print "<td style='text-align:center'><a href='https://cxc.cfa.harvard.edu/mta/CUS/Usint/poc_obsid_list_ver2.cgi\?$p_id'>$p_name</a></td>";
	...
	print '<p>Click POC to display the contact information.</p>';
	...
	for($i = 0; $i < $stotal; $i++){
        ...
        print "<td style='text-align:center'><a href='http://cda.harvard.edu/chaser/startViewer.do\?menuItem=details\&obsid=$sobsid_list[$i]'>$sobsid_list[$i]</a></td>";

        print "<td style='text-align:center'><a href='http://cda.cfa.harvard.edu/chaser/startViewer.do\?menuItem=sequenceSummary&obsid=$obsid_list[$i]'>$seqno_list[$i]</a></td>";
        ...
        ####print "<td style='text-align:center'><button type='submit' name='input_obsid' value='$sobsid_list[$i]'>$pnam</button></td>";
        print "<td style='text-align:center'><button type='submit' name='Check' value='$pnam'>$pnam</button></td>";
	...
	print "</table>";
		...
		for($i = 0; $i < $ntotal; $i++){
			...
			print "<td style='text-align:center'>$type</td>";
            print "<td style='text-align:center'><a href='http://cda.harvard.edu/chaser/startViewer.do\?menuItem=details\&obsid=$nobsid_list[$i]'>$nobsid_list[$i]</a></td>";

            print "<td style='text-align:center'><a href='http://cda.cfa.harvard.edu/chaser/startViewer.do\?menuItem=sequenceSummary&obsid=$nobsid_list[$i]'>$nseqno_list[$i]</a></td>";
