...
#-----------------------------------------------------
        ...
        [org_dict, dat_dict, nc_dict, nc_param] = initialize_dat_dicts(obsid, self.submitter)
...
#----------------------------------------------------------------------------------
    ...
    def update_db(self, form):
        ...
        fx = open('/proj/web-r2d2-v/lib/python2.7/site-packages/r2d2-v/TEST/zzz', 'w')
...
#
            #fx = open('/proj/web-r2d2-v/lib/python2.7/site-packages/r2d2-v/TEST/xxx', 'w')
            #line = 'I AM HERE: ' + str(ind) + '\n\n'
            #fx.write(line)
            #line = str(self.org_dict) + '\n'
            #fx.write(line)
            #fx.close()
...
def initialize_dat_dicts(obsid, submitter, asis='norm'):
    """
    set the initial org_dict/dat_dict
    input:  obsid       --- obsid
            submitter   --- submitter
    ...
    for key in tmp_dict.keys():
        ...
        dat_dict[key] = tmp_dict[key]
#
#--- fill up the list of non-modifiable parameters; each element has a form of [param name, value]
#
    nc_param = pdd.return_non_changerble_param_list(tmp_dict)

    for ent in nc_param:
        org_dict[ent[0]] = str(ent[1])
        dat_dict[ent[0]] = str(ent[1])
        nc_dict[ent[0]]  = str(ent[1])

    return [org_dict, dat_dict, nc_dict, nc_param]
