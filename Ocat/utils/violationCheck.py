...
#--- set a few lists
#
non_list       = ['NA', 'NULL', 'N', 'NONE', 'None', '', '\s+', None]
...
def create_normal_warning_list(dat_dict):
    """
...
def cdo_checks(dat_dict):
    ...
    for pname in cdo_check_list:
        ...
        if dat_dict[pname] != dat_dict[oname]:
            line = pname.capitalize() + ' changed from: ' + str(dat_dict[oname]) 
...
#
            if pname == 'instrument':
                if check_acis_hrc_change(dat_dict[pname],  dat_dict[oname]):
                    atemp = re.split('-', dat_dict[pname])
                    line = 'Make sure to fill all required ' + atemp[0] + ' parameters'
                    atemp = re.split('-', dat_dict[oname])
                    line = line + ' and nullify all ' + dat_dict[oname] + ' parameters.'
                    cdo_list.append(line)
            chk += 1
#
...
#
    try:
        ora  = float(dat_dict['org_dra'])
        odec = float(dat_dict['org_ddec'])
